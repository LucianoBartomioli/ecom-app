...
function validateAttachment(file, kind) {
  ...
  if (kind === "pdf" && tipo.indexOf("pdf") === -1) return { ok: false, error: "Ese archivo no es un PDF." };
  if (kind === "video" && tipo.indexOf("video") !== 0) return { ok: false, error: "Ese archivo no es un video." };
  if (!file.size) return { ok: false, error: "El archivo está vacío." };
  if (file.size > MAX_FILE_BYTES) return { ok: false, tooBig: true,
    error: "Pesa " + fmtBytes(file.size) + " y el máximo que puedo guardar acá es " + fmtBytes(MAX_FILE_BYTES) + "." };
  return { ok: true };
}
...
async function sPutFile(id, payload) {
  if (!hasStore()) { memStore[FILE_KEY(id)] = payload; return true; }
  try { await window.storage.set(FILE_KEY(id), JSON.stringify(payload), true); return true; }
  catch (e) { return false; }
}
...
function BriefEditor({ brief, config, onSave, onClose, onDelete }) {
  ...
  const subirPdf = async (file) => {
    const v = validateAttachment(file, "pdf");
    ...
    } catch (e) { setErrs((p) => ({ ...p, pdf: e.message })); }
    setCargando("");
  };
  ...
  const subirRefs = async (fileList) => {
    ...
    for (const file of files) {
      const v = validateAttachment(file, "video");
      ...
      try {
        ...
        const id = uid("f");
        setPend((p) => ({ ...p, [id]: { name: file.name, type: file.type, data } }));
        nuevos.push({ id, name: file.name, size: file.size, type: file.type, seconds: secs || 0 });
  ...
  const save = async () => {
    ...
    for (const id of ids) {
      const ok = await sPutFile(id, pend[id]);
      if (!ok) { setGuardando(false); setErrs((p) => ({ ...p, pdf: "No pude guardar los archivos. Probá con uno más liviano." })); return; }
    }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 18 }}>
      <div>
        ...
        <Field label="Brief en PDF" error={errs.pdf}
          hint={"Se guarda dentro de la app. Los editores lo abren sin salir de acá. Máximo " + fmtBytes(MAX_FILE_BYTES) + "."}>
          {f.pdf
            ...
            : <div className="drop" onClick={() => pdfRef.current && pdfRef.current.click()}
              ...
              <div style={{ fontFamily: "Archivo", fontWeight: 800, fontSize: 14 }}>
                {cargando === "pdf" ? "Subiendo…" : "Soltá el PDF del brief"}</div>
            </div>}
        ...
        <Field label="Videos a modelar" error={errs.refs}
          hint={"Los ejemplos que querés que copien. Máximo " + fmtBytes(MAX_FILE_BYTES) + " por video — para los pesados, el conector de Drive."}>
          <div className="drop" onClick={() => vidRef.current && vidRef.current.click()}
            onDragOver={(e) => e.preventDefault()}
            onDrop={(e) => { e.preventDefault(); subirRefs(e.dataTransfer.files); }}>
            <div style={{ fontFamily: "Archivo", fontWeight: 800, fontSize: 14 }}>
              {cargando === "refs" ? "Subiendo…" : "Soltá los videos de ejemplo"}</div>
...
function AttachViewer({ item, kind }) {
  ...
  return <div style={{ marginTop: 10 }}>
    <div className="split" style={{ justifyContent: "space-between", marginBottom: 6 }}>
      ...
      <a className="btn sm ghost" href={url} target="_blank" rel="noreferrer">Abrir aparte</a>
    </div>
    {kind === "pdf"
...
function DeliveryEditor({ delivery, state, identity, onSave, onClose, onDelete }) {
  ...
  const addVariant = () => setF((p) => ({ ...p, variants: (p.variants || []).concat([{ id: uid("v"), seconds: parseDuration(p.secondsInput) || 0, title: "", driveUrl: "" }]) }));
      ...
      <div>
        <Field label="El archivo de video">
          <div className="drop" data-on={arrastrando ? "1" : "0"}
            ...
            <div style={{ fontFamily: "Archivo", fontWeight: 800, fontSize: 14 }}>
              {midiendo ? "Leyendo el video…" : nombreArchivo ? nombreArchivo : "Soltá el video acá"}</div>
            <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>
              {nombreArchivo
                ? "Duración leída del archivo. Subilo a Drive con el nombre de abajo."
        ...
        <div className="card" style={{ background: "var(--panel2)" }}><div className="card-b" style={{ padding: 13 }}>
          <div className="eyebrow" style={{ marginBottom: 7 }}>Renombrá el archivo así antes de subirlo a Drive</div>
        ...
        <div style={{ marginTop: 15 }}>
          ...
          {variants.length === 0
            ? <div className="hint" style={{ marginTop: 0 }}>
              Mismo video con otro arranque. Cada uno suma {config.variantSeconds} segundos, no un video entero.</div>
            : variants.map((v, i) => (
              <div key={v.id || i} className="split" style={{ gap: 7, marginBottom: 6 }}>
                <span className="chip" style={{ minWidth: 62, justifyContent: "center" }}>HOOK {variantLabel(i + 1)}</span>
                <input className="inp" style={{ flex: 1, minWidth: 80 }} value={v.title || ""} placeholder="nota opcional"
                  onChange={(e) => setVariant(i, { title: e.target.value })} />
                {politica.link === "libre" && <input className="inp" style={{ width: 130 }} value={v.driveUrl || ""}
                  placeholder="link de Drive" onChange={(e) => setVariant(i, { driveUrl: e.target.value })} />}
...
function BulkUpload({ state, identity, onSave, onClose, notify }) {
  ...
  const ingest = useCallback(async (fileList) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      medidos.push({ name: f.name, seconds: medida.seconds || 0, size: f.size,
        unread: !medida.seconds, motivo: medida.motivo || null });
    }
    const nuevos = groupFilesByHook(medidos).map((g) => ({
      ...
      durationSource: f.seconds ? "file" : null, driveUrl: "", title: f.title,
      })),
    }));
  ...
  const guardar = () => {
    ...
    const nuevos = preview.map((x, i) => ({
      ...
      driveUrl: x.group.files[0].driveUrl || "", status: common.status, date: common.date,
  ...
  return <Modal title="Cargar varios videos" onClose={onClose} wide
    footer={<>
      ...
      <button className="btn" onClick={onClose}>Cancelar</button>
      <button className="btn primary" disabled={!groups.length || reading} onClick={guardar}>
...
function SettingsScreen({ state, actions, notify, identity, onDirty }) {
  ...
  const exportAll = () => {
    const blob = new Blob([JSON.stringify({ config, briefs, deliveries, meta, exportedAt: new Date().toISOString() }, null, 2)], { type: "application/json" });
    const a = document.createElement("a");
    ...
    <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))" }}>
      ...
      <div className="card">
        ...
        <div className="card-b">
          ...
          <Field label="URL del servidor de sincronización" hint="Opcional. Solo si corrés el conector de Drive y Meta que va aparte.">
            <input className="inp" value={c.backendUrl} onChange={(e) => set("backendUrl", e.target.value)} placeholder="https://tu-servidor.com" />
          </Field>
...
export default function App() {
  ...
  const commitList = useCallback(async (key, setter, updater) => {
    ...
    setter(merged);
    const ok = await sSet(key, merged);
    setSync(hasStore() ? (ok ? "ok" : "err") : "local");
  }, []);
  ...
  const actions = useMemo(() => ({
    ...
    addMeta: async (rows) => {
      ...
      setMeta(res.rows);
      const ok = await sSet(KEYS.meta, res.rows);
      setSync(hasStore() ? (ok ? "ok" : "err") : "local");
      notify(res.agregadas + " filas nuevas" + (res.reemplazadas ? " · " + res.reemplazadas + " actualizadas" : "") + ".");
    },
    saveConfig: async (c) => { setConfig(c); const ok = await sSet(KEYS.config, c); setSync(hasStore() ? (ok ? "ok" : "err") : "local"); },
    wipe: async () => {
