...
### 2·B · OAuth desde la propia app (lo más simple)

No hace falta terminal ni copiar tokens: apretás un botón en la app,
autorizás en Google, y el permiso se guarda solo en la base.

1. **Creá un cliente OAuth de tipo Aplicación web.** Ojo acá: el que ya
   tenías es de tipo *Aplicación de escritorio* y no sirve para este flujo,
   porque no permite registrar una dirección de vuelta. En la consola:
   **Google Auth Platform → Clientes → Crear cliente → Aplicación web.**

2. En ese cliente, en **URI de redireccionamiento autorizados**, agregá
   exactamente:
   ...
   Con tu dominio real, sin barra al final. Si te equivocás en una letra,
   Google devuelve `redirect_uri_mismatch`.

3. Copiá el nuevo `client_id` y `client_secret` y cargalos en Vercel como
   `GOOGLE_CLIENT_ID` y `GOOGLE_CLIENT_SECRET`. Agregá también
   `GOOGLE_DRIVE_ROOT_FOLDER_ID` con el id de tu carpeta madre. Redesplegá.

4. En la pantalla de consentimiento (**Google Auth Platform → Público**),
   apretá **Publicar aplicación** para pasarla de Prueba a Producción. Si
   queda en Prueba, Google vence el permiso cada 7 días.

5. Entrá a la app como administrador, andá a **Ajustes** y apretá
   **Conectar Drive**. Aceptá con la cuenta dueña de la carpeta. Cuando
   vuelvas, arriba a la derecha va a decir *Drive conectado*.

> **Sobre la pantalla de "aplicación no verificada".** Como el permiso de
> Drive es sensible, Google muestra una advertencia. Entrás igual desde
> *Configuración avanzada → Ir a (nombre de la app)*. Hasta 100 usuarios no
> hace falta verificar nada.

> **Si Google no manda el token de larga duración**, es porque ya habías
> autorizado antes. Entrá a `myaccount.google.com/permissions`, quitale el
> acceso a la app y repetí el paso 5. La propia app te lo dice si pasa.

**El método viejo con terminal sigue disponible.** Si preferís no crear un
cliente web, corré `node sacar-token-google.js` y cargá el resultado en
`GOOGLE_REFRESH_TOKEN`. Funciona igual; el flujo desde la app solo existe
para ahorrarte ese paso.
