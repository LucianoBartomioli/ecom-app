...
import { leerRefreshGuardado } from "./kv";
...
async function tokenPorRefresh(refreshToken) {
...
export async function accessToken(alcance) {
  ...
  // Primero el permiso que se autorizó desde la propia app.
  const guardado = await leerRefreshGuardado();
  if (guardado) return tokenPorRefresh(guardado);

  if (process.env.GOOGLE_REFRESH_TOKEN) return tokenPorRefresh();
  throw new Error(
    "Todavía no autorizaste Google. Entrá a /api/google/auth desde la app para conectarlo."
  );
}

/** Para mostrar en pantalla qué camino quedó activo. */
export async function metodoConfigurado() {
  if (leerCuentaDeServicio(process.env.GOOGLE_SERVICE_ACCOUNT_JSON)) return "cuenta_de_servicio";
  if (await leerRefreshGuardado()) return "oauth_desde_la_app";
